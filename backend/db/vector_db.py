...
try:
    ...
    logger.warning(" Sử dụng NumPy dự phòng (CHẬM, KHÔNG KHUYẾN NGHỊ CHO PRODUCTION)")
    ...
    def read_index(path):
        ...
        if not path.exists():
            logger.warning(f" Không tìm thấy file index: {path}")
            return None
...
class VectorDatabase:
        ...
        if self.index_path.exists() and self.metadata_path.exists():
            try:
                ...
                if self.index is None:
                    logger.warning(" Index đã tải nhưng là None, đang tạo mới")
        ...
        if self.use_hnsw:
            try:
                if hasattr(faiss, 'IndexHNSWFlat'):
                    ...
                    self._index_type = 'FLAT'
                    logger.warning(" HNSW không khả dụng, sử dụng Flat")
            except Exception as e:
    ...
    def add_document_embeddings_batch(self, doc_id: str, filename: str, 
        ...
        if not embeddings:
            logger.warning(f" Không có embeddings để thêm cho {filename}")
            return 0
    ...
    def delete_document(self, doc_id: str) -> Dict:
        ...
        logger.info(f" Đã đánh dấu {marked_count} vectors là đã xóa cho {doc_id}")
    ...
    def rebuild_index(self) -> Dict:
        ...
        if not FAISS_AVAILABLE:
            logger.warning(" Không thể rebuild mà không có FAISS")
            return {}
    ...
    def save(self):
        ...
        try:
            if not self.index:
                logger.warning(" Không có index để lưu")
                return
